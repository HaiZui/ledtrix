...
import os
import math
from ledtrix.helpers import Color, Point, darken_color, int_to_rgb_color
from ledtrix.apps import Module
import ledtrix.input as input
...
class Snake(Module):
	def __init__(self, screen):
		...
		self.snake = [Point(int(screen.height / 2), int(screen.width / 2))]
		self.dir = Point(0, -1)
	...
	def new_game(self):
		...
		self.snake = [Point(int(self.screen.width / 2), int(self.screen.height / 2))]
		self.dir = Point(0, -1)
		self.last_food = None
		...
		while time.clock() < start + t:
			self.screen.clear()
			self.screen.pixel[int(self.snake[0].x)][int(self.snake[0].y)] = int_to_rgb_color(darken_color(self.head_color, (time.clock() - start)**2 / t))
			self.screen.update()
	...
	def game_over(self):
		...
		while time.clock() < start + t:
			if not self.running:
				return
			self.screen.clear()
			self.screen.pixel[self.snake[0].x][self.snake[0].y] = int_to_rgb_color(darken_color(self.head_color, (1 - (time.clock() - start) / t)**2))
			self.screen.update()
	...
	def set_food(self):
		...
		while self.food in self.snake:
			self.food = Point(random.randint(0, self.screen.height - 1), random.randint(0, self.screen.width - 1))
				...
				for x in range(self.screen.width):
					for y in range(self.screen.height):
						...
						if (d / radius)**0.5 < (time.clock() - self.pulse_offset) / t:
							self.screen.pixel[x][y] = int_to_rgb_color(darken_color(self.food_color, 0.2 * (1 - (time.clock() - self.pulse_offset) / t)**2))
		
		for p in self.snake:
			self.screen.pixel[p.x][p.y] = int_to_rgb_color(Color(255, 255, 255))
		self.screen.pixel[self.snake[0].x][self.snake[0].y] = int_to_rgb_color(self.head_color)
		
		if self.food != None:
			self.screen.pixel[self.food.x][self.food.y] = int_to_rgb_color(darken_color(self.food_color, math.sin((time.clock() - self.pulse_offset) * 8) ** 2))
