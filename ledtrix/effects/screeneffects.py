...
class EffectRoll(Effect):
    ...
    def process(self, screen):
        screen.shape.pixel_mapping_absolute += np.array(self.shift)
        # Adjust mapping in case soome indexes were moved outside of the canvas
        screen.adjust_pixel_mapping()
...
class EffectDiffusion(Effect):
    ...
    def process(self, screen):
        ...
        screen.shape.pixel_mapping_absolute += shift
        # Adjust mapping in case soome indexes were moved outside of the canvas
        screen.adjust_pixel_mapping()
